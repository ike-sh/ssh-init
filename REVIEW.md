...
> 当前维护说明（2026-09-06）：下文是历史审查记录，其中“生产可用”、完整回滚和“80+ 用例”等结论不能作为当前版本保证。后续审查发现了无换行公钥追加、私钥交付顺序、Include 扫描及恢复失败回滚的缺口；本轮修复与验证范围见 `CHANGELOG.md` 的 v0.5.7 和 `TEST_RESULTS.md`。模拟测试及配置解析测试不等于真实 VPS 登录验证。
